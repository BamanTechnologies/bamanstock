    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-8">
        {#each ["Investors", "Merchants"] as tab}
          <button onclick={() => activeTab = tab} class="relative pb-2 text-sm font-bold {activeTab === tab ? 'text-[#4D8DEE]' : 'text-slate-400'}">
            {tab}
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-500 shadow-sm">
          <Icon iconName="icon/calendar" size={16} />
          <span>01 Jan 2024 - 07 Jan 2024</span>
        </div>
        <button class="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">
          <Icon iconName="icon/file-text" size={16} class="text-red-500" /> Export
        </button>
      </div>
    </div>

    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm min-h-[500px]">
      <div class="p-4 border-b border-slate-100 flex items-center justify-between">
        <div class="relative w-80">
          <Icon iconName="icon/search" size={18} class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input type="text" placeholder="Search" class="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 pl-10 pr-4 text-sm" />
        </div>
        <div class="flex gap-2">
          <button class="px-3 py-2 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 flex items-center gap-2">Location <Icon iconName="icon/chevron-down" size={14} /></button>
          <button class="px-3 py-2 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 flex items-center gap-2">Status <Icon iconName="icon/chevron-down" size={14} /></button>
        </div>
      </div>

      <table class="w-full text-sm">
        <thead class="bg-slate-50/80 text-slate-500 font-bold uppercase text-[11px] tracking-wider">
          <tr>
          ...
          {#each users as user}
            <tr class="hover:bg-slate-50/50">
              ...
              <td class="px-6 py-4 text-right">
                <button onclick={() => selectedUser = user} class="p-1.5 text-slate-400 hover:text-blue-500"><Icon iconName="icon/eye" size={16} /></button>
                <button class="p-1.5 text-slate-400 hover:text-blue-500"><Icon iconName="icon/user" size={16} /></button>
                <button class="p-1.5 text-slate-400 hover:text-rose-500"><Icon iconName="icon/trash" size={16} /></button>
              </td>
            </tr>
          {/each}
        </tbody>
      ...
      <div class="p-4 border-t border-slate-100 flex items-center justify-between text-slate-500 text-sm">
        <div class="flex items-center gap-2">
          Row Per Page 
          <select class="bg-slate-50 border border-slate-200 rounded px-2 py-1 outline-none">
            <option>10</option>
          </select>
          Entries
        </div>
...
{:else}
  <div class="space-y-6" in:fade>
    ...
    <button onclick={() => selectedUser = null} class="flex items-center gap-2 text-slate-500 font-medium hover:text-slate-800 transition-colors">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="text-xl font-bold text-slate-900">{activeTab === 'Merchants' ? 'Beth Richard' : selectedUser.name}</h3>
      ...
      <div class="flex gap-3">
        <button onclick={() => isResetModalOpen = true} class="px-4 py-2 border border-[#4DA0E6] text-[#4DA0E6] text-sm font-bold rounded-lg hover:bg-blue-50">
          Reset Password
        </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
      {#each (activeTab === 'Investors' ? investorStats : merchantStats) as stat}
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <p class="text-xs font-bold pt-4 border-t border-slate-50 {stat.change.includes('+') ? 'text-emerald-500' : 'text-rose-500'}">
            {stat.change}
          </p>
        </div>
      {/each}
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      {#if activeTab === 'Investors'}
        <div class="p-6 border-b border-slate-100 flex justify-between items-center">
          <h3 class="font-bold text-slate-900">Merchants Managed</h3>
          <div class="flex gap-2">
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-medium flex items-center gap-2">Location <Icon iconName="icon/chevron-down" size={12} /></button>
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-medium flex items-center gap-2">Status <Icon iconName="icon/chevron-down" size={12} /></button>
          </div>
        </div>
        <table class="w-full text-sm">
          <thead class="bg-slate-50/80 text-slate-400 text-[11px] uppercase font-bold">
            <tr>
              <th class="px-6 py-4 w-10"><input type="checkbox" class="rounded border-slate-300" /></th>
          ...
          <tbody class="divide-y divide-slate-50">
            {#each Array(8) as _, i}
              <tr class="text-slate-600 hover:bg-slate-50/50">
                ...
                <td class="px-6 py-4 font-bold text-slate-800">$156,900</td>
                <td class="px-6 py-4">785</td>
                <td class="px-6 py-4">Branch #2</td>
                <td class="px-6 py-4">
                  {#if i === 0}
                    <span class="px-2 py-0.5 rounded-full text-[10px] bg-slate-100 text-slate-500 font-bold uppercase">Invited</span>
                  {:else if i === 2}
                ...
                <td class="px-6 py-4 text-right">
                  <button class="text-slate-400 px-1 hover:text-blue-500"><Icon iconName="icon/eye" size={16} /></button>
                  <button class="text-slate-400 px-1 hover:text-rose-500"><Icon iconName="icon/trash" size={16} /></button>
                </td>
              </tr>
            {/each}
          </tbody>
      ...
      {:else}
        <div class="p-6 space-y-6">
          <div class="flex justify-between items-start">
            <div class="space-y-1">
              <p class="text-sm font-medium text-slate-500">Revenue Over Time</p>
              <p class="text-3xl font-bold text-slate-900">$690,237.90</p>
              <p class="text-xs font-bold text-slate-400">Last 12 Months <span class="text-emerald-500">+15.2%</span></p>
            </div>
            <div class="flex border border-slate-100 rounded-lg overflow-hidden bg-slate-50">
              {#each ['1D', '1W', '1M', '3M', '6M', '1Y'] as range}
                <button class="px-3 py-1.5 text-[10px] font-bold {range === '1Y' ? 'bg-blue-500 text-white' : 'text-slate-400 hover:bg-slate-100'}">{range}</button>
              {/each}
            </div>
          </div>
          <div class="h-64 w-full bg-gradient-to-t from-blue-50/50 to-transparent relative rounded-xl flex items-end justify-between px-2">
             ...
             {#each ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'] as month}
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest pb-2 z-10">{month}</span>
             {/each}
          </div>
        </div>
      {/if}
