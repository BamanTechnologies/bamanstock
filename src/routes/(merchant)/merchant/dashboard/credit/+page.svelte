...
<div class="p-6 space-y-6">
  <div class="flex items-center justify-between">
    <div
      class="flex items-center gap-2 bg-card border border-border rounded px-3 py-1.5 text-xs text-muted-foreground"
    >
    ...
    <div class="flex items-center gap-3">
      <button
        class="p-2 border border-border rounded bg-card text-red-500 hover:bg-muted"
      >
        <Icon iconName="icon/file-text" size={18} />
      </button>
      <button
        class="p-2 border border-border rounded bg-card text-muted-foreground hover:bg-muted"
      >
      ...
      <div class="bg-card border border-border rounded-lg p-5">
        ...
        <p
          class="text-xs {kpi.changeType === 'positive'
            ...
            : 'text-red-600'}"
        >
          <span class="font-medium">{kpi.change.split(" ")[0]}</span>
          {kpi.change.slice(kpi.change.indexOf(" "))}
        </p>
      </div>
    {/each}
  </div>
  ...
  <div class="bg-card border border-border rounded-lg overflow-hidden">
    <div class="p-4 border-b border-border flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <div
          class="flex items-center gap-2 bg-muted/20 border border-border rounded px-3 py-1.5 text-xs text-muted-foreground"
        >
          <Icon iconName="icon/calendar" size={14} />
          <span>01 Jan 2024 - 07 Jan 2024</span>
        </div>
        <select
          class="text-xs border border-border rounded px-2 py-1.5 bg-transparent"
        >
        ...
        <div
          class="flex items-center gap-2 border border-border rounded px-2 py-1 bg-transparent"
        >
        ...
        <thead class="bg-muted/30">
          ...
          >
            <th class="px-6 py-4 w-10 text-center"
              ><input type="checkbox" class="rounded border-border" /></th
            >
            <th class="px-4 py-4 font-medium uppercase"
              >Order ID <Icon
            ...
            <th class="px-4 py-4 font-medium uppercase"
              ...
              /></th
            >
            <th class="px-4 py-4 font-medium uppercase">Amount Paid</th>
            <th class="px-4 py-4 font-medium uppercase">Unpaid Amount</th>
            <th class="px-4 py-4 font-medium uppercase"
              >Issue Date <Icon
                iconName="icon/chevron-down"
                size={10}
              ...
              /></th
            >
            <th class="px-4 py-4 font-medium uppercase text-nowrap"
              >Due Date <Icon
                iconName="icon/chevron-down"
                size={10}
                class="inline"
          ...
          {#each creditData as row}
            <tr class="hover:bg-muted/20 transition-colors">
              <td class="px-6 py-4 text-center"
                ><input type="checkbox" class="rounded border-border" /></td
              >
              ...
              <td class="px-4 py-4 text-muted-foreground whitespace-nowrap"
                >{row.dueDate}</td
              >
              <td class="px-4 py-4">
                <span
                  class="px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap {getStatusStyles(
                    row.status,
                  )}"
                >
                  • {row.status}
              ...
              <td class="px-4 py-4 text-right flex justify-end gap-1">
                <button
                  ...
                  ><Icon iconName="icon/eye" size={14} /></button
                >
                <button
                  class="p-1.5 hover:bg-muted rounded text-muted-foreground"
                >
                  <Icon iconName={"icon/filter" as any} size={14} />
                </button>
              </td>
            </tr>
          {/each}
    ...
    <div
      class="p-4 border-t border-border flex items-center justify-between text-xs text-muted-foreground"
    >
      ...
      <div class="flex items-center gap-1">
        <button class="p-1 border border-border rounded hover:bg-muted"
          ><Icon iconName="icon/chevron-left" size={12} /></button
        >
        {#each [1, 2, 3] as page}
          <button
            class="w-6 h-6 flex items-center justify-center rounded border border-border hover:bg-muted"
            >{page}</button
          >
        {/each}
        <button
          class="w-6 h-6 flex items-center justify-center rounded bg-info text-white border-info"
          >4</button
        >
        ...
        >
        <button class="p-1 border border-border rounded hover:bg-muted"
          ><Icon iconName="icon/chevron-right" size={12} /></button
        >
      </div>
    </div>
  </div>
